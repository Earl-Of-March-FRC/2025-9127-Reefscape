...
public RobotContainer() {
  ...
  private void configureBindings() {
    ...
    operatorController.rightTrigger().whileTrue(new InstantCommand(()->{
      elevatorOffset+=ElevatorConstants.OFFSET;
    }));
    ...
    // operatorController.leftBumper().onTrue(new InstantCommand(() -> {double setpoint = elevator.getPrevSetpoint(); new ElevatorPID(elevator, () -> setpoint + elevatorOffset).schedule();}, elevator)
    //   .alongWith(Commands.runOnce(() -> elevatorOffset = 0)));
    // operatorController.rightBumper().onTrue(new InstantCommand(() -> {double setpoint = elevator.getNextSetpoint(); new ElevatorPID(elevator, () -> setpoint + elevatorOffset).schedule();}, elevator)
    //   .alongWith(Commands.runOnce(() -> elevatorOffset = 0)));

    operatorController.leftBumper().onTrue(Commands.runOnce(() -> elevatorOffset = 0)
      .andThen(new InstantCommand(() -> {double setpoint = elevator.getPrevSetpoint(); new ElevatorPID(elevator, () -> MathUtil.clamp(setpoint + elevatorOffset, ElevatorConstants.L4_POSITION, ElevatorConstants.INTAKE_POSITION)).schedule();}, elevator)));
    operatorController.rightBumper().onTrue(Commands.runOnce(() -> elevatorOffset = 0)
      .andThen(new InstantCommand(() -> {double setpoint = elevator.getNextSetpoint(); new ElevatorPID(elevator, () -> MathUtil.clamp(setpoint + elevatorOffset, ElevatorConstants.L4_POSITION, ElevatorConstants.INTAKE_POSITION)).schedule();}, elevator)));
